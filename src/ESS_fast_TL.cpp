  ...
  double qnorm_custom(double p) { return R::qnorm(p, 0.0, 1.0, 1, 0); }
  double pnorm_custom(double x) { return R::pnorm(x, 0.0, 1.0, 1, 0); }
  vec T_n_cpp(vec x) { return arma::max(x, zeros(size(x))); }

  vec get_beta(const vec& w, const vec& a, double a0, double lambda) {
  ...
  // Helper to calculate full bias vector for a single source
  vec calc_bias_vec(const vec& bs_col, int p, double lambda) {
    vec w = bs_col.subvec(0, p-1);
    vec a = bs_col.subvec(p, 2*p-1);
  ...
  List update_target_cpp(arma::vec bt_c, const arma::mat& X_T, const arma::vec& Y_T,
                         ...
                         double sd_y_T, const arma::vec& sd_y_S, int S_max) {
    ...
    vec beta_T = get_beta(w_T, a_T, a0_T, lambda_T);
    ...
    for(int s=0; s<S; s++) {
      ...
      vec bias_s = get_beta(w_s, a_s, a0_s, lambda_S(s));
    ...
    for(int k=0; k<K; k++) {
      ...
      while(n_s < S_max) {
        ...
        vec beta_T_prop = get_beta(w_prop, a_prop, a0_prop, lambda_T);
  ...
  List update_source_joint_cpp(arma::mat bs_c, // (2p+1) x S matrix
                               ...
                               const arma::mat& chol_cov_W, // Cholesky of cov_W (Lower Tri)
    ...
    for(int s=0; s<S; s++) {
      mat X = X_s_list[s];
      vec Y = Y_s_list[s];
      vec bias = calc_bias_vec(bs_c.col(s), p, lambda_S(s));
      ...
      while(n_s < S_max) {
        ...
        for(int s=0; s<S; s++) {
          ...
          if (j == 2*p) {
            ...
            vec bs_col = bs_c.col(s);
            vec bias_old = get_beta(bs_col.subvec(0, p-1),
                                    bs_col.subvec(p, 2*p-1),
                                    bs_col(2*p), lam);

            // Construct New Bias (Vector)
            vec bias_new = get_beta(bs_col.subvec(0, p-1),
                                    bs_col.subvec(p, 2*p-1),
            ...
            double thresh_prob = std::pow(pnorm_custom(a0_fixed), 1.0/lam);
            double threshold = qnorm_custom(thresh_prob);
