...
List update_target_group_aft(arma::vec bt_c,
  ...
  double threshold_T = ntl::threshold_from_a0(a0_T, lambda_T);
  vec beta_T = ntl::get_beta_group(w_T, a_T, group_map, tau, a0_T, lambda_T,
                                   slab_code, approx, k_apx);

  double current_ll_global = ntl::log_lik_aft(resid_T, C_T, sd_y_T, fam_code);
  for (int s = 0; s < S; ++s) {
    current_ll_global += ntl::log_lik_aft(resid_S[s], C_s_cpp[s], sd_y_S(s), fam_code);
  }
  ...
  for (int g = 0; g < G; ++g) {
    ...
    double act_old = ntl::activation_scalar(f_curr, threshold_T, slab_code, approx, k_apx);
    vec h_group = ntl::slab_weight_vec(w_T.elem(affected_cols), slab_code);

    while (n_s < S_max) {
      ++n_s;
      double f_prop = f_curr * std::cos(theta) + nu * std::sin(theta);
      double act_new = ntl::activation_scalar(f_prop, threshold_T, slab_code, approx, k_apx);
    ...
    for (uword pos = 0; pos < affected_cols.n_elem; ++pos) {
      ...
      while (n_w < S_max) {
        ...
        double beta_old = ntl::calc_scalar_beta(w_curr, a_T(g), threshold_T, tau,
                                                slab_code, approx, k_apx);
        double beta_new = ntl::calc_scalar_beta(w_prop, a_T(g), threshold_T, tau,
                                                slab_code, approx, k_apx);
  ...
  while (n_s < S_max) {
    ++n_s;
    double f_prop = f_curr * std::cos(theta) + nu * std::sin(theta);
    vec beta_T_prop = ntl::get_beta_group(w_T, a_T, group_map, tau, f_prop, lambda_T,
                                          slab_code, approx, k_apx);
    ...
    if (prop_ll_global > log_y_threshold) {
      bt_c(idx_a0) = f_prop;
      a0_T = f_prop;
      threshold_T = ntl::threshold_from_a0(a0_T, lambda_T);
      beta_T = beta_T_prop;
      resid_T = resid_T_prop;
      resid_S = resid_S_prop;
      current_ll_global = prop_ll_global;
...
List update_source_joint_group_aft(arma::mat bs_c,
  ...
  for (int s = 0; s < S; ++s) {
    current_ll_total += ntl::log_lik_aft(resid_S[s], C_s_cpp[s], sd_y_S(s), fam_code);
  }
  ...
  for (int g = 0; g < G; ++g) {
    ...
    while (n_s < S_max) {
      ...
      std::vector<vec> resid_S_prop(S);
      for (int s = 0; s < S; ++s) {
        double thresh_s = ntl::threshold_from_a0(bs_c(idx_a0, s), lambda_S(s));
        double act_old = ntl::activation_scalar(f_curr(s), thresh_s, slab_code, approx, k_apx);
        double act_new = ntl::activation_scalar(f_prop_row(s), thresh_s, slab_code, approx, k_apx);
        vec w_vec = bs_c.col(s).subvec(0, p - 1);
        vec h_group = ntl::slab_weight_vec(w_vec.elem(affected_cols), slab_code);
    ...
    for (uword pos = 0; pos < affected_cols.n_elem; ++pos) {
      ...
      while (n_w < S_max) {
        ...
        for (int s = 0; s < S; ++s) {
          ...
          double beta_old = ntl::calc_scalar_beta(w_curr(s), a_val, thresh_s, tau_S(s),
                                                  slab_code, approx, k_apx);
          double beta_new = ntl::calc_scalar_beta(w_prop_row(s), a_val, thresh_s, tau_S(s),
                                                  slab_code, approx, k_apx);
          double delta_beta = beta_new - beta_old;
  ...
  while (n_s < S_max) {
    ...
    for (int s = 0; s < S; ++s) {
      ...
      vec bias_old = ntl::calc_bias_vec_group(bs_col, tau_S(s), group_map, lambda_S(s),
                                              slab_code, approx, k_apx);
      bs_col(idx_a0) = f_prop_row(s);
      vec bias_new = ntl::calc_bias_vec_group(bs_col, tau_S(s), group_map, lambda_S(s),
                                              slab_code, approx, k_apx);
