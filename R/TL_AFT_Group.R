  ...
  for (i in seq_len(N)) {
    if (intercept) {
      int_res_T <- update_target_intercept_tl_aft(b0_T_curr = b0_T,
                                                  resid_T = resid_T, Y_T = Y_T, C_T = C_T,
                                                  sd_y_T = sig_T,
                                                  fam_code = fam_code,
                                                  sd_prior = 2.0)
      b0_T <- int_res_T$b0_T
    ...
    sig_T <- update_sigma_target_tl_aft(resid = resid_T, Y = Y_T, C = C_T,
                                        current_sigma = sig_T,
    ...
    if (S > 0) {
      ...
      for (s in seq_len(S)) {
        sig_s[s] <- update_sigma_source_tl_aft(resid = resid_S[[s]], Y = Y_s[[s]], C = C_s[[s]],
...
EB_SAEM_TL_Group_AFT <- function(X_T, Y_T, C_T=NULL,
  ...
  if (warm_start) {
    for (mcem_iter in seq_len(warm_start_iter)) {
      ...
      res <- ESS_Gibbs_TL_Group_AFT(X_T=X_T, Y_T=Y_T, C_T=C_T,
                                    X_s=X_s, Y_s=Y_s, C_s=C_s,
                                    group_map=group_map,
                                    bt.c=bt.c, bs.c=bs.c,
                                    lambda_T=lambda_T, lambda_s=lambda_s,
